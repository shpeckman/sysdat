...
thread_limits = system.limits.threads
...
puts "=== Block Devices ==="
system.storage.block_devices.each do |disk|
...
default_gw = system.network.routes.find { |r| r.destination == "0.0.0.0" }
puts "Default GW:  #{default_gw.try(&.gateway) || "None"}"
puts "ARP Entries: #{system.network.arp_cache.size}"
puts ""
