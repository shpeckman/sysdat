...
module Sysdat::Storage
  ...
  class MountsCollector
    include Sysdat::Collector(Array(Mount))

    def collect : Array(Mount)
      mounts = [] of Mount
      ...
      SysFS.read_lines("/proc/mounts") do |line|
        ...
        usage = Storage.filesystem(mount_point)
        mounts << Mount.new(
          device: device,
  ...
  class DiskIOCollector
    include Sysdat::Collector(Array(DiskIO))

    def collect : Array(DiskIO)
      disks = [] of DiskIO
  ...
  class BlockDevicesCollector
    include Sysdat::Collector(Array(BlockDevice))

    def collect : Array(BlockDevice)
      ...
      SysFS.children("/sys/block").each do |entry|
        ...
        devices << BlockDevice.new(
          ...
          partitions: Storage.read_partitions(base, entry),
        )
      end
  ...
  class SwapsCollector
    include Sysdat::Collector(Array(SwapDevice))

    def collect : Array(SwapDevice)
  ...
  protected def self.read_partitions(base : String, device : String) : Array(Partition)
  ...
  class Facade
    ...
    def mounts : Array(Mount)
      MountsCollector.new.collect
    end

    def disk_io : Array(DiskIO)
      DiskIOCollector.new.collect
    end

    def block_devices : Array(BlockDevice)
      BlockDevicesCollector.new.collect
    end

    def swaps : Array(SwapDevice)
      SwapsCollector.new.collect
    end
